...
GITHUB_TOKEN_EXPIRY = date(2026, 12, 20)
...
def fetch_listings():
    with sync_playwright() as p:
        ...
        for page_num in range(1, PAGES_TO_SCAN + 1):
            ...
            # До 2 попыток на страницу - раньше одиночный таймаут при
            # загрузке (не редкость на бесплатных раннерах GitHub) тихо
            # обрывал сканирование ВСЕХ следующих страниц, из-за чего
            # вместо ~250 карточек бот видел только полсотни с первой
            # страницы и мог не заметить объявление, которое туда не
            # попадало. Теперь одна неудачная попытка не приговор.
            loaded = False
            last_error = None
            for attempt in range(2):
                try:
                    ...
                    last_error = e
                    if attempt == 0:
                        print(f"Страница {page_num}: сбой с первой попытки ({e}), пробую ещё раз...")

            if not loaded:
                print(f"Не удалось загрузить страницу {page_num} за 2 попытки: {last_error}")
                if page_num == 1:
                    # Если не удалась даже первая страница - это блокировка
                    # или изменилась вёрстка, дальше сканировать бессмысленно
                    browser.close()
                    return []
                # Вторая+ страница не загрузилась даже со второй попытки -
                # пробуем следующие страницы дальше, а не сдаёмся совсем
                # (например, если сбойнула именно эта одна страница).
                continue
...
def check_token_expiry(state):
    ...
    if days_left == 0:
        ...
        text = (
            ...
            f"(20 декабря 2026). Заранее создайте новый токен и обновите "
